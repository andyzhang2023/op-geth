...
func (w *worker) generateDAGTx(signer types.Signer, txIndex int, coinbase common.Address) (*types.Transaction, error) {
	...
	txDAG.SetTxDep(txIndex, types.TxDep{Flags: new(uint8)})
	...
	tx := types.NewTransaction(nonce, receiver, big.NewInt(0), 21100, big.NewInt(0), append(types.TXDAGHeader, txDAGBytes...))
