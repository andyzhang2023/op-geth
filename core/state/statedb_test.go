...
func TestSuicide(t *testing.T) {
	...
	slotDb := NewSlotDB(state, 0, 0, unconfirmedDBs)

	addr := common.BytesToAddress([]byte("so"))
	slotDb.SetBalance(addr, big.NewInt(1))

	slotDb.SelfDestruct(addr)
	...
	hasSuicide := slotDb.HasSelfDestructed(addr)
	if !hasSuicide {
...
func TestMergeSlotDB(t *testing.T) {
	...
	oldSlotDb := NewSlotDB(state, 0, 0, unconfirmedDBs)

	newSlotDb := NewSlotDB(state, 0, 0, unconfirmedDBs)
	...
	if ok := changeList.getStateObject(addr); ok != nil {
		t.Fatalf("address should exist in StateChangeSet")
	}

	if ok := changeList.GetBalance(addr); ok != common.Big0 {
		t.Fatalf("address should exist in StateChangeSet")
	}

	if ok := changeList.GetCode(addr); ok != nil {
